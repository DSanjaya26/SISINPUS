...
$hasil = $koneksi->QUERY("SELECT * FROM `tb_buku` inner join `tb_rak` on tb_buku.kd_rak = tb_rak.kd_rak WHERE tb_buku.kd_buku = '$kd_buku';");
$rows = $hasil->fetch_All(MYSQLI_ASSOC);
foreach ($rows as $row):
    $rak = $row['kd_rak'];

 ?>  
                                        <input type="hidden" name="kd_buku" value="<?php echo $row['kd_buku']; ?>">
                                        <div class="row">
...
?>
<?php 
$hasil = $koneksi->QUERY("SELECT * FROM `tb_rak` WHERE tb_rak.kd_rak != '$rak';");
$rows = $hasil->fetch_All(MYSQLI_ASSOC);
foreach ($rows as $row):
 ?>
                                                    <option value="<?php echo $row['kd_rak']; ?>"><?php echo $row['nama_rak']; ?></option>
...
?>
                                        ...
                                        <div class="row">
                                            ...
                                            <input type="submit" class="btn btn-info btn-md" value="Update" name="UpdateBuku">
                                            </div>
                                        </div>
                                        <div class="row">
                                            <br>
                                            <label class="col-sm-offset-1">
                                                *Catatan : Cover Buku tidak perlu diisi jika tidak ada</label>
                                        </div>
                                    </div>
                            </form>
                            </div>
